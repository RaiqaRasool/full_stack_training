...
class ReportGenerator:
    ...
    def generate_yearly_extremes_report(self, year_date: datetime) -> None:
        ...
        (highest_temp_record, lowest_temp_record, highest_humidity_record) = self._weather_man.get_year_extremes(year_date)

        print(f'Highest: {highest_temp_record.max_temp}C on {highest_temp_record.date.strftime("%B %d")}')
        print(f'Lowest: {lowest_temp_record.min_temp}C on {lowest_temp_record.date.strftime("%B %d")}')
        print(f'Humidity: {highest_humidity_record.max_humidity}% on {highest_humidity_record.date.strftime("%B %d")}')
    ...
    def generate_monthly_avg_report(self, year_month: datetime) -> None:
        ...
        (highest_temp_avg, lowest_temp_avg, mean_humidity_avg) = self._weather_man.get_month_avg(year_month)
    ...
    def generate_daily_month_report(self, year_month: datetime) -> None:
        ...
        print(f"\n=== Report for {year_month.strftime('%B, %Y')} ===")
        if not weather_data:
            raise ValueError(f"No data available for the {year_month.strftime('%m-%Y')}")
