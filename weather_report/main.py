...
def parse_weather_data(weather_row) -> dict:
    ...
    for key,value in weather_row.items():
        if key in ('PKT','PKST'):
            parsed_weather_row['PKT'] = datetime.strptime(value,"%Y-%m-%d").date()
        elif key ==' Events':
    ...
    if not year_boundary_check(report_year):
        ...
        for idx,row in enumerate(data):
            if row['PKT'].year==report_year:
                if data[highest_temp_idx]['Max TemperatureC']<row['Max TemperatureC']:
                    highest_temp_idx=idx
                if data[lowest_temp_idx]['Min TemperatureC']>row['Min TemperatureC']:
                    lowest_temp_idx=idx
                if data[highest_humidity_idx]['Max Humidity']<row['Max Humidity']:
        ...
        print("Highest:",str(highest_temp_row['Max TemperatureC'])+
        'C on ',highest_temp_row['PKT'].strftime("%B %d"))
        print("Lowest:",str(lowest_temp_row['Min TemperatureC'])+
        'C on ',lowest_temp_row['PKT'].strftime("%B %d"))
        print("Humidity:",str(highest_humidity_row['Max Humidity'])+
        '% on ',highest_humidity_row['PKT'].strftime("%B %d"))
...
if __name__=="__main__":
    ...
    parser.add_argument('-c','--month_bar',
