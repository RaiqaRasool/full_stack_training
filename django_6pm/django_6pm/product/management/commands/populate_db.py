...
from product.utils.utils import generate_slug, validate_float
...
class Command(BaseCommand):
    ...
    def handle(self, *args: dict[str, Any], **kwargs: dict[str, Any]) -> None:
        ...
        for idx, item in enumerate(items):
            try:
                self.save_item(item)
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully! Saved item no. {idx+1}")
                )
    ...
    def save_brand(self, data: dict[str, str]) -> Brand:
        ...
        brand, _ = Brand.objects.get_or_create(
            slug=generate_slug(name), defaults={"name": name, "logo": data["logo"]}
        )
        return brand

    def save_category(self, categories: list[str]) -> Category:
        parent_category, _ = Category.objects.get_or_create(
            slug=generate_slug(categories[0]), defaults={"name": categories[0]}
        )
    ...
    def save_sku_color(self, color: str, product: Product) -> ProductColor:
        color_obj, _ = ProductColor.objects.get_or_create(color=color, product=product)
        return color_obj
    ...
    def save_sku_image(self, image: str, color: ProductColor) -> ProductColorImage:
        image_obj, _ = ProductColorImage.objects.get_or_create(image=image, color=color)
        return image_obj
